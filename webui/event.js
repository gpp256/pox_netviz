...
// initialize ui
function initUI() {
	$(".change-pos-enable").click(function(){
		var parent = $(this).parents('.switch');
		if (window.confirm('移動モードOnに切り替えます。よろしいですか?')) {
			$('.change-pos-disable',parent).removeClass('selected');
			$(this).addClass('selected');
			g.change_pos_mode=1;
		} else {
		}
	});
	$(".change-pos-disable").click(function(){
		var parent = $(this).parents('.switch');
		if (window.confirm('移動モードOffに切り替えます。よろしいですか?')) {
			$('.change-pos-enable',parent).removeClass('selected');
			$(this).addClass('selected');
			g.change_pos_mode=0;
			g.scrollX = 0.0; g.scrollY = 0.0;
		} else {
		}
	});
	$('#change-pos-enable').addClass('selected');
	$('#change-pos-disable').removeClass('selected');
	g.change_pos_mode=1;
	togglePerspective(0);

	initSwitchInfo('00-00-00-00-00-01');
	$('#flowlist').css('font-size', 'xx-small');
	$('#port_status').css('font-size', 'xx-small');

	$('#ui-tab').tabs({selected:1});
	$('#ui-tab .ui-tabs-nav').removeClass('ui-widget-header').addClass('my-ui-widget-header');
...
function updateDisplayTime() {
	...
	display_time.setTime(display_time.getTime()-60000);
	$('#display_time').text('Display Time: '+
		$.sprintf("%4d-%02d-%02d %02d:%02d:%02d", 
			display_time.getFullYear(), 
			display_time.getMonth()+1, 
			display_time.getDate(),
			display_time.getHours(),
